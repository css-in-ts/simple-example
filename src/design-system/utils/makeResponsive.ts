import config, { ResponsiveDeviceTypes } from "../config";
...
export function makeResponsive<StyleType = string>({
  ...
  if (beginAt && endAt) {
    return `
      @media (min-width: ${
        config.breakpoints[beginAt]
      }px) and (max-width: ${config.breakpoints[endAt] - 1}px) {
        ${style}
      }
    `;
  }
  if (!beginAt && endAt) {
    return `
      @media (max-width: ${config.breakpoints[endAt] - 1}px) {
        ${style}
      }
    `;
  }
  if (beginAt && !endAt) {
    return `
      @media (min-width: ${config.breakpoints[beginAt]}px) {
        ${style}
      }
    `;
  }

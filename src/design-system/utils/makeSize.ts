...
const sizeArr = Object.keys(config.size.fontSizeScaleMap);
...
export const convertToUnits: ConvertToUnits = (
  value,
  unit = config.size.sizeUnits
) => {
  if (unit === "em") return em(value, config.size.documentFontSize);
  if (unit === "rem") return rem(value, config.size.documentFontSize);
...
const createSizes = () => {
  ...
  const snappedFontSizeMap = Object.entries(modularScaleFontSizeMap).reduce(
    (accum, [key, value]) => ({
      ...accum,
      [key]: snapToGrid(value, config.size.baselineGrid)
    }),
    {} as Sizes
  );
  ...
  const snappedLineHeightMap = Object.entries(modularScaleFontSizeMap).reduce(
    (accum, [key, value]) => ({
      ...accum,
      [key]: snapToGrid(value, config.size.baselineGrid, {
        multiplier: config.size.lineHeight
      })
    }),
    {} as Sizes
  );
...
export const convertHeadingSizeToSize = (fontSize: Size | SizeHeadings): Size =>
  config.font.headingSizeMap[fontSize as SizeHeadings] || (fontSize as Size);

export const makeSize = (size: SizeProperties): string => {
  if (typeof size === "string") {
    const sanitizedSize = convertHeadingSizeToSize(size);
    return sizeMap.size[sanitizedSize][config.size.sizeUnits];
  }
